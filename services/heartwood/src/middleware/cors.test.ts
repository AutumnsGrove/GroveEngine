...
describe("corsMiddleware", () => {
	...
	describe("allowed origins", () => {
		it("allows requests from explicit ALLOWED_ORIGINS", async () => {
			...
			expect(res.headers.get("Access-Control-Allow-Origin")).toBe("https://heartwood.grove.place");
	...
	describe("credentials handling", () => {
		it("sets Access-Control-Allow-Credentials only for allowed origins", async () => {
			...
			expect(invalidRes.headers.get("Access-Control-Allow-Credentials")).toBeNull();
		});
	});
});
...
describe("validateOriginForClient", () => {
	...
	it("returns true when origin is in client's allowed_origins", async () => {
		const mockClient = {
			id: "client-123",
			client_id: "test-app",
			allowed_origins: JSON.stringify(["https://app.example.com", "https://staging.example.com"]),
			// other required fields...
		};

		(getClientByClientId as ReturnType<typeof vi.fn>).mockResolvedValue(mockClient);

		const result = await validateOriginForClient(mockEnv.DB, "test-app", "https://app.example.com");
	...
	it("returns false when origin is NOT in client's allowed_origins", async () => {
		...
		const result = await validateOriginForClient(mockEnv.DB, "test-app", "https://evil.com");
	...
	it("parses JSON allowed_origins correctly", async () => {
		...
		for (const origin of allowedOrigins) {
			const result = await validateOriginForClient(mockEnv.DB, "test-app", origin);
			expect(result).toBe(true);
		}
	});
	...
	it("calls getClientByClientId with correct parameters", async () => {
		...
		await validateOriginForClient(mockEnv.DB, "my-client-id", "https://app.com");

		expect(getClientByClientId).toHaveBeenCalledWith(mockEnv.DB, "my-client-id");
