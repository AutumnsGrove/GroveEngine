...
describe("createRateLimiter", () => {
	...
	it("uses anonymous fallback key when getKey returns null", async () => {
		(checkRateLimit as ReturnType<typeof vi.fn>).mockResolvedValue({
			allowed: true,
			remaining: 10,
			resetAt: new Date(Date.now() + 60000),
		...
		const res = await app.request("/test", {}, mockEnv);
		expect(res.status).toBe(200);
		// checkRateLimit is called with the anonymous fallback key
		expect(checkRateLimit).toHaveBeenCalledOnce();
		expect(checkRateLimit).toHaveBeenCalledWith(expect.anything(), "test:anonymous", 10, 60);
	});
...
describe("checkRouteRateLimit", () => {
	...
	it("returns allowed=true when under limit", async () => {
		...
		const result = await checkRouteRateLimit({} as any, "token", "127.0.0.1:test-app", 20);
	...
	it("constructs key from prefix and keyPart", async () => {
		...
		expect(checkRateLimit).toHaveBeenCalledWith(expect.anything(), "magic_ip:192.168.1.1", 10, 60);
	});
	...
	it("uses default window of RATE_LIMIT_WINDOW", async () => {
		...
		expect(checkRateLimit).toHaveBeenCalledWith(expect.anything(), "test:key", 5, 60);
	});
});
