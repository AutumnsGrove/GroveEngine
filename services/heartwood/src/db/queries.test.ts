...
import { validateSubdomain } from "../types/branded.js";

/**
 * Shorthand for tests — asserts the input is a valid subdomain and returns
 * the branded value. Fails loudly if the test data itself is malformed.
 */
const sd = (raw: string) => {
	const brand = validateSubdomain(raw);
	if (!brand) throw new Error(`test subdomain "${raw}" failed validation`);
	return brand;
};
...
describe("isActiveTenant", () => {
	it("returns true for active tenant", async () => {
		...
		const result = await isActiveTenant(mockDb as unknown as D1Database, sd("autumn"));
	...
	it("returns false for non-existent tenant", async () => {
		...
		const result = await isActiveTenant(mockDb as unknown as D1Database, sd("doesnotexist"));

		expect(result).toBe(false);
	});

	it("passes pre-normalized branded subdomain straight to the query", async () => {
		// Normalization is now the job of validateSubdomain() at the trust
		// boundary — isActiveTenant trusts its Subdomain input.
		const mockDb = createMockDb({ 1: 1 });
		await isActiveTenant(mockDb as unknown as D1Database, sd("AUTUMN"));
...
describe("cleanupOldAuditLogs", () => {
	...
	it("should delete logs older than retention period", async () => {
		...
		expect(mockDb.prepare).toHaveBeenCalledWith("DELETE FROM audit_log WHERE created_at < ?");
	});
	...
	it("should use 90-day default retention", async () => {
		...
		expect(Math.abs(cutoffDate.getTime() - ninetyDaysAgo.getTime())).toBeLessThan(1000);
	});
