...
export function splitPreviewIntoLines(
	...
	for (const word of words) {
		...
		if (wouldBe.length <= maxCharsPerLine[currentLine]) {
			...
			lines[currentLine] = word.slice(0, maxCharsPerLine[currentLine] - 3) + "...";
			currentLine++;
		} else {
...
export type Variant = "default" | "forest" | "workshop" | "midnight" | "knowledge";
	...
	switch (variant) {
		...
		default:
			return {
				...
				muted: slate[400],
				glass: "rgba(15, 23, 42, 0.5)",
				glassBorder: "rgba(22, 163, 74, 0.25)",
			};
	}
}
...
const ALLOWED_DOMAINS = new Set(["grove.place", "cdn.grove.place", "imagedelivery.net"]);
...
export function resolveExternalUrl(base: URL, relative: string | undefined): string | undefined {
...
export function extractExternalMetaContent(html: string, property: string): string | undefined {
	const ogPatterns = [
...
export function extractExternalFavicon(html: string, baseUrl: URL): string | undefined {
	const patterns = [
...
export function parseExternalOGMetadata(html: string, url: URL): Omit<OGMetadata, "fetchedAt"> {
	return {
		url: url.href,
