...
describe('pathUtils.ts', () => {
	describe('svgToPercent', () => {
		it('should convert SVG coordinates to percentages', () => {
			const result = svgToPercent({ x: 50, y: 25 }, { width: 100, height: 100 });
			expect(result.x).toBe(50);
			expect(result.y).toBe(25);
		});

		it('should handle non-100 viewBox dimensions', () => {
			const result = svgToPercent({ x: 200, y: 100 }, { width: 400, height: 200 });
			expect(result.x).toBe(50);
		...
		it('should handle max point', () => {
			const result = svgToPercent({ x: 100, y: 100 }, { width: 100, height: 100 });
			expect(result.x).toBe(100);
			expect(result.y).toBe(100);
		});

		it('should handle fractional results', () => {
			const result = svgToPercent({ x: 33, y: 66 }, { width: 100, height: 100 });
