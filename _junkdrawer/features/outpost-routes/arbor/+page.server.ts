...
export const load: PageServerLoad = async ({ parent, platform, request }) => {
	...
	const [statusRes, whitelistRes, historyRes] = await Promise.all([
		...
		heartwoodFetch(platform, request, "/minecraft/history?limit=20").catch(() => null),
	]);
...
export const actions: Actions = {
	start: async ({ request, locals, platform }) => {
		...
		if (!platform?.env?.AUTH) return fail(500, { error: "Service not available" });
	...
	command: async ({ request, locals, platform }) => {
		...
		const blocked = ["stop", "restart", "kill", "ban-ip", "op", "deop", "pardon-ip"];
	...
	whitelistAdd: async ({ request, locals, platform }) => {
		...
		const res = await heartwoodFetch(platform, request, "/minecraft/whitelist", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
	...
	whitelistRemove: async ({ request, locals, platform }) => {
		...
		if (!res.ok) {
			const data = await res.json<any>().catch(() => ({}));
			return fail(res.status, {
				error: data.error_description || data.error || "Failed to remove player",
			});
		}
