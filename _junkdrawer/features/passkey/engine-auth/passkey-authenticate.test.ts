...
function createMockPublicKeyCredential(
	...
	(mock as unknown as Record<string, unknown>).isUserVerifyingPlatformAuthenticatorAvailable = vi
		.fn()
		.mockResolvedValue(options.platformAuthenticatorAvailable ?? true);

	if (options.conditionalMediationAvailable !== undefined) {
		(mock as unknown as Record<string, unknown>).isConditionalMediationAvailable = vi
			.fn()
		...
		function setupAuthenticationMocks(
			...
			const credentialsGet = options.credentialError
				...
				: vi
						.fn()
						.mockResolvedValue(
							"credentialResult" in options ? options.credentialResult : createMockCredential(),
						);
