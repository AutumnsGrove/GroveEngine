...
export function createTerrariumState() {
	...
	const complexityUsage = $derived<number>(
		Math.min(calculateSceneComplexity(scene.assets) / TERRARIUM_CONFIG.complexity.maxComplexity, 1),
	);
	...
	function updateAsset(id: string, updates: Partial<PlacedAsset>): void {
		...
		scene.assets[index] = {
			...
			...updates,
			id,
			position: updates.position ? { ...updates.position } : scene.assets[index].position,
