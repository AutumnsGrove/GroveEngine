...
import { detectProvider, isMusicUrl } from "./hum/detect.js";
...
export function humPlugin(md: MarkdownIt): void {
	md.core.ruler.push("hum_music_links", (state) => {
		...
		for (let i = 0; i < blockTokens.length; i++) {
			...
			if (nonEmptyTokens.length >= 2 && nonEmptyTokens[0].type === "link_open") {
				const result = isStandaloneMusicLink(children, children.indexOf(nonEmptyTokens[0]));
				...
				if (result) {
					...
					let paraOpenIdx = i - 1;
					while (paraOpenIdx >= 0 && blockTokens[paraOpenIdx].type !== "paragraph_open") {
						paraOpenIdx--;
					}
