...
import { humPlugin } from "./hum-plugin";
...
describe("humPlugin - bare URL detection", () => {
	...
	it("transforms a bare Spotify URL into a hum card", () => {
		const result = md.render("https://open.spotify.com/track/75FEaRjZTKLhTrFGsfMUXR");
	...
	it("transforms a bare Apple Music URL into a hum card", () => {
		const result = md.render("https://music.apple.com/us/album/hounds-of-love/1558627166");
		expect(result).toContain('class="hum-card"');
...
describe("humPlugin - mixed content", () => {
	...
	it("does not transform a music URL inline with other text", () => {
		const input = "Listen to https://open.spotify.com/track/75FEaRjZTKLhTrFGsfMUXR right now";
		const result = md.render(input);
...
describe("humPlugin - XSS prevention", () => {
	...
	it("escapes special characters in URLs", () => {
		...
		const result = md.render('https://open.spotify.com/track/abc"onload="alert(1)');
