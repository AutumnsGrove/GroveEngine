...
// Test data helpers
const createMarkdownContent = (title: string, date: string, content: string = "# Test"): string => {
...
describe("markdown.ts - Comprehensive Tests", () => {
	...
	describe("generateHeadingId()", () => {
		...
		it("handles special markdown characters", () => {
			expect(generateHeadingId("Using `code` in header")).toBe("using-code-in-header");
	...
	describe("processAnchorTags()", () => {
		it("converts anchor comment to span element", () => {
			...
			expect(result).toContain('<span class="anchor-marker" data-anchor="my-anchor"></span>');
		});
	...
	describe("parseMarkdownContent()", () => {
		it("extracts frontmatter data", () => {
			const content = createMarkdownContent("Test Post", "2024-01-01", "# Content");
		...
		it("converts markdown to HTML", () => {
			const content = createMarkdownContent("Test", "2024-01-01", "# Heading\n\nParagraph");
			const result = parseMarkdownContent(content);
		...
		it("extracts headers from markdown", () => {
			const content = createMarkdownContent("Test", "2024-01-01", "# Header 1\n## Header 2");
			const result = parseMarkdownContent(content);
	...
	describe("parseMarkdownContentSanitized()", () => {
		it("sanitizes HTML content", () => {
			const content = createMarkdownContent("Test", "2024-01-01", "<script>alert(1)</script>");
		...
		it("extracts frontmatter", () => {
			const content = createMarkdownContent("Test Title", "2024-01-01", "# Content");
			const result = parseMarkdownContentSanitized(content);
			expect(result.data).toBeDefined();
			expect(result.data).toHaveProperty("title");
		});
	...
	describe("getItemBySlug()", () => {
		...
		it("returns post with content and headers", () => {
			const modules: ModuleMap = {
				"/posts/test.md": createMarkdownContent("Test", "2024-01-01", "# Header 1\n## Header 2"),
			};
		...
		it("includes headers in post", () => {
			const modules: ModuleMap = {
				"/posts/test.md": createMarkdownContent("Test", "2024-01-01", "# Main\n## Sub"),
			};
			const result = getItemBySlug("test", modules);
			expect(result?.headers).toHaveLength(2);
		});
	...
	describe("getPageByFilename()", () => {
		...
		it("returns page with content and headers", () => {
			const modules: ModuleMap = {
				"/pages/home.md": createMarkdownContent("Home", "2024-01-01", "# Welcome\n## Section"),
			};
			const result = getPageByFilename("home.md", modules);
			expect(result).not.toBeNull();
		...
		it("returns page with sanitized content", () => {
			const modules: ModuleMap = {
				"/pages/home.md": createMarkdownContent("Home", "2024-01-01", "<script>alert(1)</script>"),
			};
			const result = getPageByFilename("home.md", modules);
			expect(result?.content).not.toContain("<script");
		});
...
describe("Inline elements in headings (regression: commit 8264e99)", () => {
	...
	it("generateHeadingId strips inline markup for ID generation", () => {
		...
		expect(generateHeadingId("***bold italic*** heading")).toBe("bold-italic-heading");
	});
