-- Migration: Gallery Tables to Engine DB
-- Description: Consolidate gallery tables from curios DB to engine DB
-- Date: 2026-04-18
--
-- Context: Gallery tables were initially created in the curios DB (migration 031),
-- but the upload pipeline writes to engine DB. This migration consolidates everything
-- into engine DB for architectural consistency.
--
-- Includes all columns from:
-- - 031_gallery_curio.sql (base tables)
-- - curios/003_gallery_thumbnails.sql (thumbnail_r2_key, dominant_color, aspect_ratio)
...
CREATE TABLE IF NOT EXISTS gallery_images (
    ...
    width INTEGER,
    height INTEGER,
    aspect_ratio REAL DEFAULT NULL,    -- Added in curios/003_gallery_thumbnails.sql

    -- Performance features (added in curios/003_gallery_thumbnails.sql)
    thumbnail_r2_key TEXT DEFAULT NULL, -- Pre-generated thumbnail path
    dominant_color TEXT DEFAULT NULL,   -- Hex color for instant placeholder
