...
import { generateCSRFToken, validateCSRFToken, validateCSRF } from "$lib/utils/csrf.js";
...
describe("Full-Stack: CSRF Validation", () => {
	...
	describe("Origin-based CSRF (validateCSRF)", () => {
		...
		it("accepts same-origin request (host matches origin)", () => {
			const request = createRequestWithHeaders("https://autumn.grove.place/arbor/posts", {
				headers: {
		...
		it("allows HTTP for localhost", () => {
			const request = createRequestWithHeaders("http://localhost:5173/arbor/posts", {
		...
		it("prevents cross-tenant CSRF (tenant1 → tenant2)", () => {
			const request = createRequestWithHeaders("https://victim-tenant.grove.place/arbor/settings", {
				headers: {
...
describe("Full-Stack: Rate Limiting", () => {
	...
	describe("rateLimit() function", () => {
		it("allows first request and sets count to 1", async () => {
			const result = await rateLimit(kv as unknown as KVNamespace, "user:test-1", {
				limit: 5,
				windowSeconds: 60,
			});
		...
		it("decrements remaining on each call", async () => {
			...
			const r1 = await rateLimit(kv as unknown as KVNamespace, "user:dec-test", opts);
			expect(r1.remaining).toBe(2);

			const r2 = await rateLimit(kv as unknown as KVNamespace, "user:dec-test", opts);
			expect(r2.remaining).toBe(1);

			const r3 = await rateLimit(kv as unknown as KVNamespace, "user:dec-test", opts);
		...
		it("blocks after limit is reached", async () => {
			...
			const r3 = await rateLimit(kv as unknown as KVNamespace, "user:block-test", opts);
		...
		it("isolates different keys", async () => {
			...
			const result = await rateLimit(kv as unknown as KVNamespace, "user:B", opts);
			expect(result.allowed).toBe(true);
		...
		it("resets after window expires", async () => {
			...
			const r1 = await rateLimit(kv as unknown as KVNamespace, "user:expire-test", opts);
			expect(r1.allowed).toBe(true);

			// Second request blocked
			const r2 = await rateLimit(kv as unknown as KVNamespace, "user:expire-test", opts);
			...
			const r3 = await rateLimit(kv as unknown as KVNamespace, "user:expire-test", opts);
		...
		it("uses namespace prefix in KV keys", async () => {
			...
			expect(keys.some((k) => k.includes("custom") && k.includes("ns-test"))).toBe(true);
		});
	});
...
describe("Full-Stack: Webhook HMAC Verification", () => {
	 ...
	 * This mirrors what payment providers do when sending webhook events.
	 */
