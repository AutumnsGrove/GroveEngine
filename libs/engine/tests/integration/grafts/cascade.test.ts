...
import { getEnabledGrafts, isInGreenhouse } from "../../../src/lib/platform/feature-flags/index.js";
import type { FeatureFlagsEnv } from "../../../src/lib/platform/feature-flags/types.js";
...
describe("Grafts Cascade Integration", () => {
	...
	describe("Page data cascade simulation", () => {
		...
		it("handles missing grafts gracefully in page components", async () => {
			...
			const grafts = await getEnabledGrafts({ tenantId, inGreenhouse: false }, env);
...
describe("image_uploads_enabled graft", () => {
	...
	it("cascades to page data like editor and gallery would consume", async () => {
		...
		const galleryUploadsEnabled = pageData.grafts?.image_uploads_enabled ?? false;
