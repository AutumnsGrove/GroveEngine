...
import { createMockRequestEvent, createMockD1, seedMockD1 } from "../helpers/index.js";
...
describe("Webhook Endpoint - Configuration Validation", () => {
	it("should return 500 when STRIPE_SECRET_KEY is missing", async () => {
		...
		try {
			...
			expect((error.body as any).message).toContain("Payment service is temporarily unavailable");
		}
	});
	...
	it("should return 500 when STRIPE_WEBHOOK_SECRET is missing", async () => {
		...
		try {
			...
			const error = err as HttpError;
			expect(error.status).toBe(500);
			expect((error.body as any).message).toContain("Service temporarily unavailable");
		}
	});
...
describe("Webhook Endpoint - Verification", () => {
	it("should return 400 when webhook verification fails", async () => {
		...
		mockHandleWebhook.mockResolvedValue(mockWebhookFailure("Invalid signature"));
...
describe("Webhook Endpoint - Idempotency", () => {
	it("should return 200 with 'Already processed' for duplicate events", async () => {
		...
		mockHandleWebhook.mockResolvedValue(mockWebhookSuccess("subscription.created", "stripe-evt-1"));
...
describe("Webhook Endpoint - Subscription Events", () => {
	it("should process subscription.created event", async () => {
		...
		mockHandleWebhook.mockResolvedValue(mockWebhookSuccess("subscription.created", "stripe-sub-1"));
	...
	it("should process subscription.updated event", async () => {
		...
		mockHandleWebhook.mockResolvedValue(mockWebhookSuccess("subscription.updated", "stripe-sub-2"));
...
describe("Webhook Endpoint - Data Persistence", () => {
	...
	it("should store webhook event payload in database", async () => {
		...
		// Stripe payloads don't match the structured format the sanitizer expects,
