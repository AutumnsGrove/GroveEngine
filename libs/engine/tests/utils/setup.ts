...
// Manually extend expect — the auto-setup via "@testing-library/jest-dom/vitest"
// breaks with vitest 4.x due to a module resolution mismatch
...
class MockDurableObjectStub {
	...
	private async handleTenantRequest(path: string, method: string, req: Request): Promise<Response> {
		...
		if (path === "/config" && method === "GET") {
			const config = this.storage.get("config") as Record<string, unknown> | undefined;
		...
		if (path === "/config" && method === "PUT") {
			...
			const current = (this.storage.get("config") as Record<string, unknown>) || {};
		...
		if (path.startsWith("/drafts/")) {
			...
			const draftsMap = (this.storage.get("draftsMap") || {}) as Record<string, DraftData>;
	...
	private async handlePostMetaRequest(
		...
		if (path === "/meta/init" && method === "POST") {
			...
			let meta = this.storage.get("meta") as Record<string, unknown> | undefined;
		...
		if (path === "/view" && method === "POST") {
			const meta = this.storage.get("meta") as Record<string, unknown> | undefined;
			...
			const sessions = (this.storage.get("sessions") || {}) as Record<string, number>;
		...
		if (path === "/presence" && method === "GET") {
			...
			const activeReaders = Object.values(sessions).filter((t) => t > cutoff).length;
