...
describe("Integration: Wisp Generation Path", () => {
	it("should run full pipeline for text generation task", async () => {
		fetchMock.mockResolvedValueOnce({
			ok: true,
			json: async () =>
				mockOpenRouterResponse('{"grammar":[],"tone":"professional","readability":"clear"}'),
		});
...
describe("Integration: Timeline BYOK Path", () => {
	it("should thread tenantApiKey to Authorization header", async () => {
		fetchMock.mockResolvedValueOnce({
			ok: true,
			json: async () => mockOpenRouterResponse("Today I worked on the Lumen integration."),
		});
	...
	it("should allow model override for BYOK users", async () => {
		fetchMock.mockResolvedValueOnce({
			ok: true,
			json: async () => mockOpenRouterResponse("Done.", "anthropic/claude-sonnet-4"),
		});
...
describe("Integration: Petal Image Classification Path", () => {
	...
	it("should format multimodal messages correctly for OpenRouter API", async () => {
		fetchMock.mockResolvedValueOnce({
			ok: true,
			json: async () => mockOpenRouterResponse('{"category": "nature", "confidence": 0.9}'),
		});
...
describe("Integration: Error Handling", () => {
	it("should propagate provider errors with context", async () => {
		...
		await expect(lumen.run({ task: "generation", input: "Hello" })).rejects.toThrow(
			/Rate limit exceeded|All providers failed/,
		);
	});
	...
	it("should throw when Lumen is disabled", async () => {
		...
		await expect(lumen.run({ task: "generation", input: "Test" })).rejects.toThrow(
			"Lumen is disabled",
		);
