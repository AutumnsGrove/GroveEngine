...
export class QuotaExceededError extends LumenError {
	...
	constructor(task: LumenTask, limit: number, used: number) {
		super(`Quota exceeded for task "${task}": ${used}/${limit} requests used`, "QUOTA_EXCEEDED", {
			task,
			retryable: false,
		});
...
export class ProviderError extends LumenError {
	...
	constructor(provider: LumenProviderName, message: string, statusCode?: number, cause?: unknown) {
...
export class AllProvidersFailedError extends LumenError {
	...
	constructor(
		...
		super(`All providers failed for task "${task}". Attempts: ${summary}`, "ALL_PROVIDERS_FAILED", {
			task,
			retryable: true,
			cause: attempts,
		});
