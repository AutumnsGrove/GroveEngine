...
describe("LumenError", () => {
	...
	describe("toUserMessage - User-Friendly Messages", () => {
		...
		it("should return friendly message for PROVIDER_TIMEOUT", () => {
			...
			expect(error.toUserMessage()).toBe("The AI service is taking too long. Please try again.");
		});
		...
		it("should return friendly message for DISABLED", () => {
			...
			expect(error.toUserMessage()).toBe("AI features are not available for your account.");
		});
...
describe("ProviderError", () => {
	...
	it("should preserve original cause", () => {
		...
		const error = new ProviderError("openrouter", "Connection failed", undefined, originalError);
...
describe("Error Inheritance", () => {
	...
	it("all errors should have name property", () => {
		...
		expect(new QuotaExceededError("generation", 100, 100).name).toBe("QuotaExceededError");
		expect(new ProviderError("openrouter", "test").name).toBe("ProviderError");
		expect(new ProviderTimeoutError("openrouter", 30000).name).toBe("ProviderTimeoutError");
		expect(new AllProvidersFailedError("generation", []).name).toBe("AllProvidersFailedError");
	});
});
