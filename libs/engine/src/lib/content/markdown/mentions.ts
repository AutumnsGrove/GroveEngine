...
import { buildGroveUrl } from "../../utils/grove-url.js";
...
export function mentionsPlugin(md: MarkdownIt): void {
	...
	md.core.ruler.push("grove_mentions", (state) => {
		...
		for (let i = 0; i < blockTokens.length; i++) {
			...
			const hasAt = blockToken.children.some((t) => t.type === "text" && t.content.includes("@"));
			if (!hasAt) continue;

			// Process children, splitting text tokens around mentions
			blockToken.children = processInlineChildren(blockToken.children, state.Token);
		}
	});
}
