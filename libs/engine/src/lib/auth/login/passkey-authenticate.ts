...
function getErrorMessage(error: unknown): string {
	if (error instanceof Error) {
		...
		if (error.message.includes("no credential") || error.message.includes("empty allow list")) {
...
export async function authenticateWithPasskey(
	...
	try {
		...
		const authOptions = (await optionsResponse.json()) as AuthenticationOptionsResponse;
