...
import { RateLimiter, RateLimitError, withRateLimit, DEFAULT_RATE_LIMITS } from "./rate-limit";
...
describe("withRateLimit", () => {
	...
	it("should call wrapped function when under limit", async () => {
		...
		const wrapped = withRateLimit(limiter, "token", (userId: string) => userId, mockFn);
