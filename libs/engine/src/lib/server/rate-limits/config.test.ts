...
describe("TIER_RATE_LIMITS", () => {
	const allTiers: SubscriptionTier[] = ["wanderer", "seedling", "sapling", "oak", "evergreen"];
...
describe("ENDPOINT_RATE_LIMITS", () => {
	...
	it("AI endpoints have daily windows", () => {
		expect(ENDPOINT_RATE_LIMITS["ai/draft"].windowSeconds).toBe(86400);
	});
});
...
describe("getEndpointLimit", () => {
	it("returns correct limit for known endpoints", () => {
		...
		const draftLimit = getEndpointLimit("POST", "/api/ai/draft");
		expect(draftLimit.limit).toBe(20);
		expect(draftLimit.windowSeconds).toBe(86400);
	});
...
describe("ENDPOINT_MAP", () => {
	...
	it("maps AI endpoints correctly", () => {
		expect(ENDPOINT_MAP["POST:/api/ai/draft"]).toBe("ai/draft");
	});
});
