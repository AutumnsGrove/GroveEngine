...
function extractTableName(sql: string): string | null {
	...
	const patterns = [/FROM\s+(\w+)/i, /INTO\s+(\w+)/i, /UPDATE\s+(\w+)/i, /TABLE\s+(\w+)/i];
 ...
 */
function getOperationType(sql: string): "INSERT" | "UPDATE" | "DELETE" | "DDL" | "QUERY" {
...
export class SafeDatabase {
	...
	private logAudit(entry: Omit<AuditLogEntry, "timestamp" | "dryRun">): void {
		if (this.config.auditLog) {
			const fullEntry: AuditLogEntry = {
				...
				params: this.config.redactParams ? entry.params?.map(() => "[REDACTED]") : entry.params,
	...
	async execute(
			...
			if (isComplexQuery) {
				...
				if (countSql !== sql) {
					...
					try {
						...
						const limit =
							operation === "DELETE" ? this.config.maxDeleteRows : this.config.maxUpdateRows;
		...
		return {
			...
			changes: (result.meta as D1Meta).changes ?? 0,
		};
	}
	...
	get unsafeDb(): D1DatabaseOrSession {
		console.warn("[SAFETY] Accessing unsafeDb bypasses safety checks. Use with caution.");
		return this.db;
	}
}
 ...
 * ```
 */
export function withSafetyGuards(db: D1DatabaseOrSession, config?: SafetyConfig): SafeDatabase {
...
export const AGENT_SAFE_CONFIG: SafetyConfig = {
	...
	protectedTables: ["users", "tenants", "subscriptions", "payments", "sessions"],
};
