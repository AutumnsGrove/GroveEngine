...
describe("User Service", () => {
	...
	describe("getUserByEmail", () => {
		...
		it("matches regardless of case (#1580)", async () => {
			// GroveAuth/Google claims aren't guaranteed to match the case stored
			// in `users.email` — the lookup must be case-insensitive or visitors
			// silently fail to resolve their own account on other tenants' sites.
			const user = await getUserByEmail(db, "TEST@Example.com");
	...
	describe("getUserFromSession", () => {
		...
		it("calls fetch with correct auth header", async () => {
			...
			expect(mockFetch).toHaveBeenCalledWith("https://auth.example.com/userinfo", {
				headers: {
		...
		it("returns user on successful token validation", async () => {
			...
			const user = await getUserFromSession(db, "test-token", "https://auth.example.com");
		...
		it("returns null on 401 unauthorized", async () => {
			...
			const user = await getUserFromSession(db, "expired-token", "https://auth.example.com");
	...
	describe("updateUserDisplayName", () => {
		it("updates user display name", async () => {
			const changes = await updateUserDisplayName(db, "user-1", "New Display Name");
		...
		it("updates display name for inactive user", async () => {
			const changes = await updateUserDisplayName(db, "user-2", "Inactive User Name");
		...
		it("returns 0 for non-existent user", async () => {
			const changes = await updateUserDisplayName(db, "non-existent", "New Name");
	...
	describe("Integration: Session Flow", () => {
		it("validates token, finds user, and returns active user", async () => {
			...
			const user = await getUserFromSession(db, "valid-token", "https://auth.example.com");
	...
	describe("Integration: Query Functions", () => {
		...
		it("update functions return proper change counts", async () => {
			...
			const displayNameChanges = await updateUserDisplayName(db, "user-1", "New Name");
		...
		it("handles multiple users with different statuses correctly", async () => {
			...
			const inactiveUser = await getUserFromValidatedSession(db, "grove-auth-2");
