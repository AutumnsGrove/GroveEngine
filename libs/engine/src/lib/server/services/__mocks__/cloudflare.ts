 ...
 */
function createMockD1Session(tables: Map<string, MockRow[]>): D1DatabaseSession {
	const createStatement = (sql: string) => {
		let boundParams: unknown[] = [];
...
function filterRows(
	...
	return rows.filter((row) => {
		...
		for (const condition of conditions) {
			// LOWER(column) = ? — case-insensitive comparisons (e.g. email lookups, #1580)
			const lowerEqMatch = condition.match(/LOWER\((\w+)\)\s*=\s*\?/i);
			if (lowerEqMatch) {
				const col = lowerEqMatch[1];
...
export function createMockKV(): KVNamespace & { _store: Map<string, KVEntry> } {
	...
	const kv = {
		...
		get: vi.fn(async (key: string, typeOrOptions?: string | { type?: string }) => {
 ...
 */
export function advanceKVTime(kv: ReturnType<typeof createMockKV>, ms: number): void {
