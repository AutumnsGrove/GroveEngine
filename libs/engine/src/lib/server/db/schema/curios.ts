 ...
 * gallery, guestbook, hit counters, polls, link gardens,
...
export const timelineSummaries = sqliteTable(
	"timeline_summaries",
	{
...
export const timelineActivity = sqliteTable(
	"timeline_activity",
	{
		tenantId: text("tenant_id").notNull(),
...
export const journeySnapshots = sqliteTable(
	"journey_snapshots",
	{
		id: text("id").primaryKey(),
		tenantId: text("tenant_id").notNull(),
...
export const journeySummaries = sqliteTable("journey_summaries", {
	...
	snapshotId: text("snapshot_id")
		.notNull()
		.references(() => journeySnapshots.id, { onDelete: "cascade" }),
...
export const galleryImages = sqliteTable(
	...
	(table) => [uniqueIndex("idx_gallery_images_tenant_r2").on(table.tenantId, table.r2Key)],
);
...
export const galleryTags = sqliteTable(
	...
	(table) => [uniqueIndex("idx_gallery_tags_tenant_slug").on(table.tenantId, table.slug)],
);

export const galleryImageTags = sqliteTable(
	"gallery_image_tags",
	{
		...
		tagId: text("tag_id")
			.notNull()
			.references(() => galleryTags.id, { onDelete: "cascade" }),
		createdAt: integer("created_at").default(sql`(strftime('%s', 'now'))`),
	},
	(table) => [uniqueIndex("idx_gallery_image_tags_pk").on(table.imageId, table.tagId)],
);

export const galleryCollections = sqliteTable(
	"gallery_collections",
	{
		id: text("id").primaryKey(),
		tenantId: text("tenant_id").notNull(),
		...
		slug: text("slug").notNull(),
		description: text("description"),
		coverImageId: text("cover_image_id").references(() => galleryImages.id, {
			onDelete: "set null",
		}),
	...
	(table) => [uniqueIndex("idx_gallery_collections_tenant_slug").on(table.tenantId, table.slug)],
);

export const galleryCollectionImages = sqliteTable(
...
export const hitCounters = sqliteTable(
	"hit_counters",
	{
		id: text("id").primaryKey(),
		tenantId: text("tenant_id").notNull(),
	...
	(table) => [uniqueIndex("idx_hit_counters_tenant_path").on(table.tenantId, table.pagePath)],
);

export const hitCounterVisitors = sqliteTable(
	"hit_counter_visitors",
	{
		id: integer("id").primaryKey({ autoIncrement: true }),
		tenantId: text("tenant_id").notNull(),
		pagePath: text("page_path").notNull().default("/"),
...
export const linkGardenItems = sqliteTable("link_garden_items", {
	...
	gardenId: text("garden_id")
		.notNull()
		.references(() => linkGardens.id, { onDelete: "cascade" }),
	tenantId: text("tenant_id").notNull(),
...
export const pollVotes = sqliteTable(
	...
	(table) => [uniqueIndex("idx_poll_votes_unique").on(table.pollId, table.voterHash)],
);
...
export const tenantBadges = sqliteTable(
	"tenant_badges",
	{
		id: text("id").primaryKey(),
		tenantId: text("tenant_id").notNull(),
	...
	(table) => [uniqueIndex("idx_tenant_badges_unique").on(table.tenantId, table.badgeId)],
);
...
export const bookmarks = sqliteTable("bookmarks", {
	...
	shelfId: text("shelf_id")
		.notNull()
		.references(() => bookmarkShelves.id, { onDelete: "cascade" }),
	url: text("url").notNull(),
	title: text("title").notNull(),
	author: text("author"),
	description: text("description"),
