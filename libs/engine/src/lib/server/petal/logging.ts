...
export async function logSecurityEvent(
	...
	try {
		...
		logGroveError("Petal", PETAL_ERRORS.LOG_SECURITY_EVENT_FAILED, { cause: err });
	}
}
...
export async function getRecentUserEvents(
  ...
  `;
	...
	try {
		...
		logGroveError("Petal", PETAL_ERRORS.QUERY_USER_EVENTS_FAILED, { userId, cause: err });
		return [];
	}
}
...
export async function getUserBlockCount(
	...
	try {
		...
		logGroveError("Petal", PETAL_ERRORS.QUERY_BLOCK_COUNT_FAILED, { userId, cause: err });
		return 0;
	}
}
...
export async function getRecentBlocksByCategory(
	...
	try {
		...
		logGroveError("Petal", PETAL_ERRORS.QUERY_BLOCKS_BY_CATEGORY_FAILED, { cause: err });
		return [];
	}
}
 ...
 * Default: 90 days
 */
export async function cleanupOldLogs(db: D1Database, retentionDays: number = 90): Promise<number> {
	try {
		const result = await db
			.prepare(
		...
		logGroveError("Petal", PETAL_ERRORS.LOG_CLEANUP_FAILED, { retentionDays, cause: err });
		return 0;
	}
}
