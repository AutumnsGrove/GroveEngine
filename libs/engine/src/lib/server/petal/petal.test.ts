 ...
 */
function createMockAI(response: { category: string; confidence: number } | string): Ai {
 ...
 */
function createTestEnv(aiResponse?: { category: string; confidence: number }): PetalEnv {
	return {
		AI: createMockAI(aiResponse || { category: "appropriate", confidence: 0.95 }),
		DB: createMockDB(),
...
describe("Blocked Category Coverage", () => {
	...
	it.each(blockedCategories)("should block %s category with high confidence", async (category) => {
...
describe("Error Handling", () => {
	...
	it("should handle AI timeout with appropriate error", async () => {
		const env = createTestEnv();
		// Simulate timeout
		(env.AI as { run: ReturnType<typeof vi.fn> }).run.mockRejectedValue(new Error("AbortError"));
		const image = createMockImage();
