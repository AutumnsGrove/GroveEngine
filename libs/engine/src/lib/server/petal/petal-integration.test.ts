...
import { createLumenClient } from "$lib/ai/lumen/index.js";
import { MODELS } from "$lib/ai/lumen/config.js";
...
describe("Integration: Petal classifyWithLumen → LumenClient", () => {
	...
	it("should construct correct data URI and send to provider", async () => {
		fetchMock.mockResolvedValueOnce({
			ok: true,
			json: async () => mockOpenRouterVisionResponse('{"category": "nature", "confidence": 0.88}'),
		});
		...
		expect(message.content[1].image_url.url).toBe("data:image/png;base64,SGVsbG8=");
	});
	...
	it("should convert Uint8Array to base64 and send through pipeline", async () => {
		fetchMock.mockResolvedValueOnce({
			ok: true,
			json: async () =>
				mockOpenRouterVisionResponse('{"category": "appropriate", "confidence": 0.95}'),
		});
	...
	it("should use image task routing (Gemini Flash primary)", async () => {
		fetchMock.mockResolvedValueOnce({
			ok: true,
			json: async () =>
				mockOpenRouterVisionResponse('{"category": "appropriate", "confidence": 0.9}'),
		});
	...
	it("should fail-open when provider returns unparseable classification", async () => {
		fetchMock.mockResolvedValueOnce({
			ok: true,
			json: async () =>
				mockOpenRouterVisionResponse("I cannot analyze this image due to safety restrictions."),
		});
	...
	it("should reject invalid images before reaching the provider", async () => {
		...
		// Invalid MIME type
		await expect(classifyWithLumen("data", "application/json", lumen)).rejects.toThrow(
			"Unsupported image type",
		);

		// Empty data
		await expect(classifyWithLumen("", "image/jpeg", lumen)).rejects.toThrow("Empty image data");
	...
	it("should handle provider failure gracefully", async () => {
		...
		await expect(classifyWithLumen("data", "image/jpeg", lumen)).rejects.toThrow();
	});
});
