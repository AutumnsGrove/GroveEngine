...
import type { ClassificationResult, PetalCategory, PetalDecision, PetalContext } from "./types.js";
...
export async function checkFashionContext(
	...
	if (category === "swimwear" || category === "underwear" || category === "revealing") {
		return {
			appropriate: false,
...
export async function classifyImageBatch(
	...
	for (let i = 0; i < images.length; i += CONCURRENCY) {
		...
		const batchResults = await Promise.all(
			batch.map((img) => runLayer2(img.image, img.mimeType, img.hash, context, options)),
		);
