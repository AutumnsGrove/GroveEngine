...
import { computeContentHash, getUserBlockCount, hasViolationPattern } from "./logging.js";
 ...
 */
export async function scanImage(input: PetalScanInput, env: PetalEnv): Promise<PetalResult> {
	...
	try {
		...
		// Layer 1: CSAM Detection (MANDATORY)
		// ========================================================================
		const csamResult = await runLayer1(imageData, mimeType, contentHash, options);
		...
		if (classificationResult.decision !== "allow") {
			return {
				...
				blockedAt: "layer2",
				message: classificationResult.reason || getRejectionMessage(classificationResult.category),
		...
		// Layer 3: Sanity Checks
		// ========================================================================
		const sanityResult = await runLayer3(imageData, mimeType, contentHash, context, options);
		...
		if (!sanityResult.valid) {
			return {
				...
				decision: "block",
				blockedAt: "layer3",
				message: sanityResult.reason || "This image is not suitable for this feature.",
...
export async function verifyOutput(
	...
	const outputResult = await runLayer4(generatedImage, mimeType, contentHash, options);
	...
	if (!outputResult.safe) {
		return {
			...
			blockedAt: "layer4",
			message: outputResult.reason || "Generated image did not meet safety standards.",
...
export { runLayer3, quickDimensionCheck, quickFileSizeCheck } from "./layer3-sanity.js";
export { runLayer4, generateWithRetry, verifyOutfitMatch } from "./layer4-output.js";
