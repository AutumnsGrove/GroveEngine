...
export async function runLayer4(
	...
	const { db, userId, tenantId, intendedDescription, retryCount = 0, ...visionOptions } = options;
...
export async function generateWithRetry<T extends string | Uint8Array>(
	generateFn: (seed: number) => Promise<{ image: T; hash: string; mimeType: string }>,
	options: {
		ai?: Ai;
		togetherApiKey?: string;
		db?: D1Database;
	...
	for (let attempt = 0; attempt <= maxRetries; attempt++) {
		...
		// Verify the output
		const verification = await runLayer4(generated.image, generated.mimeType, generated.hash, {
			...verifyOptions,
			retryCount: attempt,
		});
		...
		if (verification.action === "reject") {
			logGroveError("Petal", PETAL_ERRORS.OUTPUT_REJECTED);
			break;
		}
