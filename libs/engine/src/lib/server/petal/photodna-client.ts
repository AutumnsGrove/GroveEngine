...
export async function scanWithPhotoDNA(
	...
	try {
		...
		const base64Image = typeof image === "string" ? image : uint8ArrayToBase64(image);
		...
		const response = await fetch(`${PHOTODNA_CONFIG.endpoint}/v1.0/Match?enhance=true`, {
			method: "POST",
			headers: {
		...
		if (err instanceof Error && err.name === "AbortError") {
			throw new PetalError("PhotoDNA request timed out", "PHOTODNA_TIMEOUT", "layer1", "photodna");
		}
 ...
 */
export function createMockPhotoDNAScan(response: PhotoDNAMatchResponse): typeof scanWithPhotoDNA {
	return async () => ({
