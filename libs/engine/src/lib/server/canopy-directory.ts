 ...
 */
export async function fetchCanopyDirectory(db: D1Database): Promise<CanopyDirectoryResult> {
    ...
    `,
		...
		.bind(...tenantIds, CANOPY_SETTING_KEYS.BANNER, CANOPY_SETTING_KEYS.CATEGORIES)
		.all<SettingsRow>();

	const settings = settingsResult.results || [];

	// Group settings by tenant
	const settingsByTenant = new Map<string, { banner?: string; categories?: string }>();
