...
import { getTiersWithFeature, type TierKey } from "../platform/config/tiers.js";

export type PlanTier = TierKey;
export type SubscriptionStatus = "active" | "past_due" | "paused" | "canceled" | "unpaid";
...
export async function getTenantSubscription(
	...
	const billing = await db
		.prepare("SELECT status, current_period_end FROM platform_billing WHERE tenant_id = ?")
		.bind(tenantId)
 ...
 */
export async function requireActiveSubscription(db: D1Database, tenantId: string): Promise<void> {
	const subscription = await getTenantSubscription(db, tenantId);
