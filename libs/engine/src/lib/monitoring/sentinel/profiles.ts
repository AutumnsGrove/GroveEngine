...
export function createThreePhaseProfile(options: {
	...
	const peakOpsPerSecond =
		config.peakSeconds === 0 ? 0 : (targetUsersAtPeak * opsPerUser) / config.peakSeconds;
...
function getOscillationOpsPerSecond(
	...
	switch (waveform) {
		...
		case "sawtooth":
			return (
				minOpsPerSecond +
				(maxOpsPerSecond - minOpsPerSecond) * ((elapsedSeconds % periodSeconds) / periodSeconds)
			);
		default:
			return midpoint;
	}
...
function getRampOpsPerSecond(profile: LoadProfile, elapsedSeconds: number): number {
	...
	const { startOpsPerSecond, endOpsPerSecond, rampUpSeconds, sustainSeconds, rampDownSeconds } =
		config;
