...
export class SentinelDO {
	...
	private async handleStatus(): Promise<Response> {
		...
		const elapsed = this.runState.startedAt ? (Date.now() - this.runState.startedAt) / 1000 : 0;
	...
	private async recordCheckpoint(elapsedSeconds: number): Promise<void> {
		...
		const checkpoint: SentinelCheckpoint = {
			...
			errorRate:
				this.runState.failedOps / Math.max(this.runState.completedOps + this.runState.failedOps, 1),
		};
	...
	private async finishTest(): Promise<void> {
		...
		this.log("Test completed", {
			...
			totalOps,
			successRate:
				totalOps > 0 ? ((this.runState.completedOps / totalOps) * 100).toFixed(1) : "0.0",
