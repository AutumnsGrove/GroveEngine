...
import { GROVE_EMAILS } from "$lib/platform/config/emails.js";
...
export async function scheduleWelcomeSequence(
	...
	const { email, name, audienceType, zephyrApiKey, zephyrUrl, from = DEFAULT_FROM } = options;
	...
	for (const { dayOffset, template, subject } of sequence) {
		try {
			...
			const scheduledAt = dayOffset === 0 ? undefined : addDays(now, dayOffset).toISOString();
			...
			if (!response.success) {
				result.errors.push(`Day ${dayOffset}: ${response.errorMessage || "Unknown error"}`);
 ...
 */
export function getNextStage(currentStage: number, audienceType: AudienceType): number {
	const sequence = SEQUENCES[audienceType];
