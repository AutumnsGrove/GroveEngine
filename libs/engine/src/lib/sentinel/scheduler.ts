...
import type { SentinelSchedule, LoadProfile } from "./types.js";
 ...
 * Delete a schedule
 */
export async function deleteSchedule(db: D1Database, scheduleId: string): Promise<void> {
	await db.prepare("DELETE FROM sentinel_schedules WHERE id = ?").bind(scheduleId).run();
}
	...
	for (const row of dueSchedules.results ?? []) {
		...
		try {
           ...
           WHERE id = ?`,
				...
				.bind(now, run.id, nextRun ? Math.floor(nextRun.getTime() / 1000) : null, now, schedule.id)
				.run();
			...
			const runner = new SentinelRunner({
				...
				onComplete: async (results) => {
					...
					if (schedule.enableMaintenanceMode) {
						await disableMaintenanceMode(clearingConfig ?? null, schedule.tenantId);
					}
...
async function enableMaintenanceMode(
	...
	try {
		...
		if (!response.ok) {
			console.error(`[Sentinel] Failed to enable maintenance mode: ${response.status}`);
		} else {
...
async function disableMaintenanceMode(
	...
	try {
		...
		if (!response.ok) {
			console.error(`[Sentinel] Failed to disable maintenance mode: ${response.status}`);
		} else {
