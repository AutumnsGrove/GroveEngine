 ...
 */
function validateUpdateColumn(column: string, allowedColumns: Set<string>): void {
...
// D1 DATABASE TYPE
// =============================================================================
// D1Database, D1PreparedStatement, D1Result are ambient globals from
// @cloudflare/workers-types. Do NOT redeclare them locally.
...
export async function updateVariant(
	...
	if (data.billingIntervalCount !== undefined) {
		validateUpdateColumn(columnMap.billingIntervalCount, VARIANT_UPDATE_COLUMNS);
...
export async function deleteVariant(db: D1Database, variantId: string): Promise<void> {
...
export async function generateOrderNumber(db: D1Database, tenantId: string): Promise<string> {
...
export async function updateCustomer(
	...
	if (data.defaultShippingAddress !== undefined) {
		validateUpdateColumn(columnMap.defaultShippingAddress, CUSTOMER_UPDATE_COLUMNS);
	...
	if (data.defaultBillingAddress !== undefined) {
		validateUpdateColumn(columnMap.defaultBillingAddress, CUSTOMER_UPDATE_COLUMNS);
...
function mapOrderRow(row: OrderRow, lineItemRows: LineItemRow[]): Order {
	return {
		...
		billingAddress: row.billing_address
			...
			: undefined,
		fulfilledAt: row.fulfilled_at ? new Date(row.fulfilled_at * 1000) : undefined,
...
function mapLineItemRow(row: LineItemRow): LineItem {
	return {
		...
		taxAmount: row.tax_amount ? { amount: row.tax_amount, currency: "usd" } : undefined,
		metadata: safeParseJson<Record<string, string>>(
			row.metadata,
			{},
...
function mapCustomerRow(row: CustomerRow): Customer {
	return {
		...
		defaultShippingAddress: row.default_shipping_address
			? safeParseJson<Address | undefined>(row.default_shipping_address, undefined, {
					context: "customer.shippingAddress",
				})
			: undefined,
		defaultBillingAddress: row.default_billing_address
			? safeParseJson<Address | undefined>(row.default_billing_address, undefined, {
