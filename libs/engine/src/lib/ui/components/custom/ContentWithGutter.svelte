<script lang="ts">
	...
	// Static loader map — keys are valid curio names, values are lazy importers
	const CURIO_LOADERS: Record<string, () => Promise<{ default: any }>> = {
		guestbook: () => import("@autumnsgrove/curios/components/CurioGuestbook.svelte"),
		poll: () => import("@autumnsgrove/curios/components/CurioPoll.svelte"),
	};
	...
	function mountCurio(el: Element): (() => void) | null {
		...
		const loader = CURIO_LOADERS[curioName];
		if (!loader) {
