...
import { evaluatePercentageRule, getUserBucket, getUserBucketSync } from "./percentage.js";
	...
	describe("evaluatePercentageRule", () => {
		describe("edge cases: percentage boundaries", () => {
			it("returns false for percentage <= 0", async () => {
				...
				const result = await evaluatePercentageRule(condition, context, "flag1");
		...
		describe("identifier selection", () => {
			it("returns false with warning when no identifier provided", async () => {
				...
				const result = await evaluatePercentageRule(condition, context, "test-flag");

				expect(result).toBe(false);
				expect(consoleSpy).toHaveBeenCalledWith(
					expect.stringContaining("No identifier for percentage rollout of flag"),
				);
				expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("test-flag"));
		...
		describe("deterministic behavior", () => {
			it("returns same result for same input", async () => {
				...
				const result1 = await evaluatePercentageRule(condition, context, flagId);
				const result2 = await evaluatePercentageRule(condition, context, flagId);
				const result3 = await evaluatePercentageRule(condition, context, flagId);
			...
			it("returns consistent results across multiple calls", async () => {
				...
				for (let i = 0; i < 5; i++) {
					const result = await evaluatePercentageRule(condition, { userId: "same-user" }, "flag1");
					results.push(result);
				}
		...
		describe("flag-specific bucketing", () => {
			it("different flags give different results for same user", async () => {
				...
				const resultFlagA = await evaluatePercentageRule(condition, context, "flagA");
				const resultFlagB = await evaluatePercentageRule(condition, context, "flagB");
			...
			it("uses flagId in hash for independence", async () => {
				...
				const result1 = await evaluatePercentageRule(condition, context, "unique-flag-1");
				const result2 = await evaluatePercentageRule(condition, context, "unique-flag-2");
		...
		describe("percentage threshold behavior", () => {
			it("respects exact threshold (bucket < percentage)", async () => {
				...
				const bucket = await getUserBucket("threshold-flag", "threshold-test-user", "");
