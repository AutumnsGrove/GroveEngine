...
describe("Tenant Rules", () => {
	...
	it("handles multiple tenants", async () => {
		const rule = createTenantRule(["tenant-1", "tenant-2", "tenant-3", "tenant-4"]);
...
describe("Tier Rules", () => {
	...
	it("handles all tier types correctly", async () => {
		const allTiersRule = createTierRule(["wanderer", "seedling", "sapling", "oak", "evergreen"]);
	...
	it("handles specific tier combinations", async () => {
		...
		expect(await evaluateRule(paidTiersRule, wandererContext, "flag-1")).toBe(false);
	});
});
...
describe("User Rules", () => {
	...
	it("handles UUID-format userIds", async () => {
		...
		expect(await evaluateRule(uuidRule, otherUuidContext, "flag-1")).toBe(false);
	});
});
...
describe("isTierAtLeast", () => {
	...
	describe("All tier combinations", () => {
		const allTiers = ["wanderer", "seedling", "sapling", "oak", "evergreen"] as const;
...
describe("getTiersAtLeast", () => {
	describe("Valid tiers", () => {
		it("returns correct tiers for wanderer tier", () => {
			...
			expect(result).toEqual(["wanderer", "seedling", "sapling", "oak", "evergreen"]);
		});
	...
	describe("Return type", () => {
		it("returns array of TierKey type", () => {
			...
			for (const tier of result) {
				expect(["wanderer", "seedling", "sapling", "oak", "evergreen"]).toContain(tier);
			}
		});
...
describe("Rule Evaluation Integration", () => {
	...
	it("handles minimal context", async () => {
		...
		const alwaysResult = await evaluateRule(alwaysRule, minimalContext, "flag-1");
		const tenantResult = await evaluateRule(tenantRule, minimalContext, "flag-1");
	...
	it("maintains tier comparison consistency", () => {
		...
		const tierChain = ["wanderer", "seedling", "sapling", "oak", "evergreen"] as const;
