...
<script lang="ts">
	...
	let { tiers, billingPeriod, onCheckout, class: className = "" }: PricingTableProps = $props();

	// Row definitions for the comparison table
	type RowType = "feature" | "limit" | "price" | "custom" | "support" | "bestfor";
	...
	<table class="w-full text-left border-collapse" aria-label="Pricing tier comparison">
			...
			<tr class="border-b-2 border-default">
				...
				{#each tiers as tier}
					...
					<th scope="col" aria-label="{tier.name} tier" class="py-4 px-3 text-center min-w-[120px]">
						...
						{/if}
						<div class="font-serif text-foreground">
						...
						<div class="text-2xl font-sans font-bold text-accent-muted">
							...
							{#if tier.monthlyPrice > 0}
								<span class="text-sm font-normal text-foreground-faint">/mo</span>
							{/if}
						</div>
						{#if tier.status === "coming_soon"}
							<span class="text-xs text-foreground-faint italic">Coming Soon</span>
		...
		<tbody class="text-sm font-sans">
			{#each tableRows as row, rowIndex}
				{@const Icon = row.icon}
				<tr class="border-b border-subtle {rowIndex % 2 === 1 ? 'bg-surface' : ''}">
					<!-- Row label -->
			...
			<tr>
				...
				{#each tiers as tier}
					<td class="py-4 px-3 text-center">
						...
						{:else}
							<span class="font-medium text-accent-muted">${Math.round(tier.annualPrice)}</span>
						{/if}
					</td>
				{/each}
			</tr>
