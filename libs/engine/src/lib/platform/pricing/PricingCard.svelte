<script lang="ts">
	...
	let priceSuffix = $derived(
		tier.monthlyPrice === 0 ? "" : billingPeriod === "monthly" ? "/mo" : "/yr",
	);

	// Status badge text
	let statusBadge = $derived(
		tier.status === "coming_soon" ? "Coming Soon" : tier.status === "future" ? "Future" : null,
	);

	// Mode-aware display values
	let displayName = $derived(groveModeStore.current ? tier.name : tier.standardName || tier.name);
