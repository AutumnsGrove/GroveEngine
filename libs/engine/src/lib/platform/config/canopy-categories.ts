 ...
 */
export function isValidCanopyCategory(category: string): category is CanopyCategory {
 ...
 */
export function parseCanopyCategories(categoriesJson: string | null | undefined): CanopyCategory[] {
	...
	try {
		...
		return parsed.filter(
			(c): c is CanopyCategory => typeof c === "string" && isValidCanopyCategory(c),
		);
	} catch {
		return [];
	}
