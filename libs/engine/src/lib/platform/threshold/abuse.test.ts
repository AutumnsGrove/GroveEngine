...
describe("abuse tracking", () => {
	...
	describe("isBanned", () => {
		it("returns false when not banned", () => {
			expect(isBanned({ violations: 3, lastViolation: 0, bannedUntil: null })).toBe(false);
		});

		it("returns true during active ban", () => {
			const future = Math.floor(Date.now() / 1000) + 3600;
			expect(isBanned({ violations: 5, lastViolation: 0, bannedUntil: future })).toBe(true);
		});

		it("returns false after ban expires", () => {
			const past = Math.floor(Date.now() / 1000) - 1;
			expect(isBanned({ violations: 5, lastViolation: 0, bannedUntil: past })).toBe(false);
		});
	});

	describe("getBanRemaining", () => {
		it("returns 0 when not banned", () => {
			expect(getBanRemaining({ violations: 0, lastViolation: 0, bannedUntil: null })).toBe(0);
		});

		it("returns remaining seconds during ban", () => {
			const bannedUntil = Math.floor(Date.now() / 1000) + 3600;
			expect(getBanRemaining({ violations: 5, lastViolation: 0, bannedUntil })).toBe(3600);
		});
	});
