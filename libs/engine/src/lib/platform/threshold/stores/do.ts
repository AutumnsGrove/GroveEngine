...
export class ThresholdDOStore implements ThresholdStore {
	...
	private handleError(error: unknown, options: ThresholdCheckOptions): ThresholdResult {
		console.error("[threshold:do] DO fetch error, failing", options.failMode ?? "open", error);
