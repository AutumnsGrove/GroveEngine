...
} from "./processing/imageProcessor";
import { validateFileSignature, isAllowedImageType } from "./validation/upload-validation";
...
describe("Integration: Filename and Path Utilities", () => {
	...
	it("combines date path and sanitized filename for full storage path", () => {
		...
		expect(fullPath).toMatch(/^photos\/2025\/06\/15\/my-photo-[a-z0-9]+\.webp$/);
	});
});
