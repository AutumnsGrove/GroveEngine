...
export const ALLOWED_EXTENSIONS = ["jpg", "jpeg", "png", "gif", "webp", "jxl", "avif"] as const;
...
export const ALLOWED_TYPES_DISPLAY = "JPG, PNG, GIF, WebP, JPEG XL, AVIF, or HEIC";
...
export function isConvertibleFormat(file: File): boolean {
	const ext = getFileExtension(file.name);
	return CONVERTIBLE_FORMATS.some((format) => format.ext === ext || format.mime === file.type);
}
...
export function isBrowserRenderable(file: File): boolean {
	...
	if (BROWSER_RENDERABLE_TYPES.includes(file.type as (typeof BROWSER_RENDERABLE_TYPES)[number])) {
		return true;
	}
	...
	if (
		ext &&
		BROWSER_RENDERABLE_EXTENSIONS.includes(ext as (typeof BROWSER_RENDERABLE_EXTENSIONS)[number])
	) {
		return true;
	}
 ...
 */
export function getNonRenderableInfo(file: File): (typeof NON_RENDERABLE_FORMATS)[number] | null {
	const ext = getFileExtension(file.name);
	if (!ext) return null;

	return (
		NON_RENDERABLE_FORMATS.find((format) => format.ext === ext || format.mime === file.type) ?? null
	);
}
...
export function getActionableUploadError(serverMessage: string): string {
	...
	// Invalid file type
	if (msg.includes("file type") || msg.includes("unsupported") || msg.includes("invalid type")) {
	...
	if (msg.includes("forbidden") || msg.includes("403") || msg.includes("csrf")) {
		return "Upload blocked — your session may have expired. Please refresh the page.";
	}

	// Service unavailable
	if (msg.includes("503") || msg.includes("service unavailable") || msg.includes("temporarily")) {
...
export async function normalizeFileForUpload(file: File): Promise<{
	...
	const extMatches = ext !== null && extensionMatchesMimeType(ext, detectedMime);
...
export function getUploadStrategy(file: File): {
	...
	if (!isBrowserRenderable(file)) {
		return {
			...
			warning: "This file type may have limited browser support. Uploading original format.",
		};
	}
