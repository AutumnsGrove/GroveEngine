...
const JXL_CODESTREAM_BYTES = new Uint8Array([0xff, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
...
describe("upload-validation constants", () => {
	...
	describe("FILE_SIGNATURES", () => {
		...
		it("contains GIF87a signature", () => {
			...
			expect(gifSignatures.some((sig) => sig.join(",") === "71,73,70,56,55,97")).toBe(true);
		});

		it("contains GIF89a signature", () => {
			const gifSignatures = FILE_SIGNATURES["image/gif"];
			expect(gifSignatures.some((sig) => sig.join(",") === "71,73,70,56,57,97")).toBe(true);
		});

		it("contains WebP RIFF signature", () => {
			expect(FILE_SIGNATURES["image/webp"]).toBeDefined();
			expect(FILE_SIGNATURES["image/webp"][0]).toEqual([0x52, 0x49, 0x46, 0x46]);
		});
		...
		it("contains JXL container signature", () => {
			const jxlSigs = FILE_SIGNATURES["image/jxl"];
			expect(jxlSigs.some((sig) => sig.join(",") === "0,0,0,12,74,88,76,32")).toBe(true);
		});
	});
...
describe("validateFileSignature - SECURITY CRITICAL", () => {
	describe("JPEG validation", () => {
		...
		it("accepts Progressive JPEG (SOF2 0xC2)", () => {
			const progressiveJpeg = new Uint8Array([0xff, 0xd8, 0xff, 0xc2, 0x00, 0x10]);
		...
		it("rejects empty buffer", () => {
			expect(validateFileSignature(new Uint8Array([]), "image/jpeg")).toBe(false);
		});
	});
	...
	describe("JXL validation", () => {
		it("accepts JXL codestream signature", () => {
			expect(validateFileSignature(JXL_CODESTREAM_BYTES, "image/jxl")).toBe(true);
		});

		it("accepts JXL container signature", () => {
			expect(validateFileSignature(JXL_CONTAINER_BYTES, "image/jxl")).toBe(true);
		});
...
describe("validateImageFileDeep - async deep validation", () => {
	...
	it("detects file corruption (incorrect magic bytes)", async () => {
		const corruptData = new Uint8Array([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
...
describe("getActionableUploadError", () => {
	describe("feature flag disabled", () => {
		...
		it("maps 'limited beta' to rollout message", () => {
			const result = getActionableUploadError("Image uploads are in limited beta");
			expect(result).toContain("aren't available yet");
		});

		it("maps 'not enabled' to rollout message", () => {
			const result = getActionableUploadError("Feature not enabled for this tenant");
			expect(result).toContain("aren't available yet");
		});
	});
