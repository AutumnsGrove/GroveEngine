...
> extends DurableObject<TEnv> {
	...
	protected async onWebSocketError(_ws: WebSocket, _error: unknown): Promise<void> {
		// No-op by default
	}
	...
	async fetch(request: Request): Promise<Response> {
		try {
			...
			const match = matchRoute(method as LoomRoute["method"], url.pathname, routes);
	...
	protected async emit<T>(
		...
		if (!queue || typeof queue.send !== "function") {
			...
			throw new Error(`Queue binding "${queueBinding}" not found. Add it to wrangler.toml.`);
		}
	...
	protected async workflow<T>(
		...
		if (!binding || typeof binding.create !== "function") {
			...
			throw new Error(`Workflow binding "${workflowBinding}" not found. Add it to wrangler.toml.`);
		}
