<script lang="ts">
  ...
  let formMessage = $state<{ type: "success" | "error"; text: string } | null>(
    null,
  );
  ...
  let hasMore = $state(entries.length < data.total);
  let loadingMore = $state(false);
  let currentOffset = $state(entries.length);
  ...
  async function submitEntry() {
    ...
    try {
      const res = await fetch("/api/curios/guestbook", { // csrf-ok
        method: "POST",
...
<svelte:head>
  ...
  <meta
    name="description"
    content="Sign the guestbook — leave a message, say hello."
  />
</svelte:head>
  ...
  <GlassCard class="sign-card">
    ...
    >
      ...
      <div class="form-footer">
        {#if formMessage}
          <p class="form-message {formMessage.type}" role="status" aria-live="polite">{formMessage.text}</p>
        {/if}
