<script lang="ts">
  ...
  import { Download, Mail, Package, Clock, AlertTriangle, CheckCircle, Archive, XCircle } from "lucide-svelte";
  ...
  function getPhaseLabel(status: string): string {
    switch (status) {
      ...
      case 'querying': return 'Gathering your blooms...';
      case 'assembling': return 'Bundling everything together...';
      case 'uploading': return 'Almost there...';
      case 'notifying': return 'Sending you a heads up...';
  ...
  async function startExport() {
    starting = true;
    try {
      const result = await api.post<{ exportId: string; status: string }>(
        '/api/export/start',
        { includeImages, deliveryMethod },
      );
      ...
      toast.success('Export started! ' + (deliveryMethod === 'email' ? "We'll email you when it's ready." : "Hang tight while we bundle your data."));
    } catch (err: any) {
  ...
  let hasActiveExport = $derived(
    activeExportId && activeStatus &&
    !['complete', 'failed', 'expired'].includes(activeStatus)
  );
</script>
...
<div class="export-page">
  <header class="page-header">
    ...
    <p class="page-description">
      You own your content. Download everything — posts, pages, and images — as a
      tidy zip file with standard Markdown files you can take anywhere. Data portability
  ...
  <GlassCard variant="default" class="mb-6">
    ...
    <p class="size-estimate">
      ...
      Estimated export: {data.counts.posts} posts, {data.counts.pages} pages{#if includeImages}, {data.counts.media} images{/if}
  ...
  {#if hasActiveExport}
    <GlassCard variant="default" class="mb-6">
      ...
      <div class="progress-section" aria-live="polite" aria-atomic="true">
        ...
        <div class="progress-bar-container" role="progressbar" aria-valuenow={activeProgress} aria-valuemin={0} aria-valuemax={100} aria-label="Export progress">
  ...
  {#if activeExportId && activeStatus === 'complete' && deliveryMethod === 'download'}
    <GlassCard variant="default" class="mb-6">
      ...
      <Button
        variant="primary"
        onclick={() => { window.location.href = `/api/export/${activeExportId}/download`; }}
      >
        <Download class="btn-icon" aria-hidden="true" />
        Download Zip
      </Button>
    ...
    <GlassCard variant="default" class="mb-6">
      <div class="error-banner" role="alert">
        ...
        <div>
          ...
          <p>Something went wrong while preparing your export. Please try again — if the issue persists, reach out to support.</p>
        </div>
      </div>
    </GlassCard>
  {/if}
            ...
            <div class="export-meta">
              ...
              {#if exp.itemCounts}
                <span class="export-counts">
                  {exp.itemCounts.posts} posts, {exp.itemCounts.pages} pages{#if exp.includeImages}, {exp.itemCounts.images} images{/if}
                </span>
              {/if}
            </div>
            <div class="export-actions">
              {#if exp.status === 'complete'}
                <Button
                  variant="secondary"
                  size="sm"
                  onclick={() => { window.location.href = `/api/export/${exp.id}/download`; }}
                >
