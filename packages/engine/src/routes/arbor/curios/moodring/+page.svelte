    ...
    {#if data.logEntries.length > 0}
      <div class="mood-timeline">
        {#each data.logEntries as entry}
          <div class="timeline-dot" style="background-color: {entry.color}" title="{entry.mood}{entry.note ? ` — ${entry.note}` : ''}"></div>
        {/each}
      </div>
    {:else}
...
<style>
  .moodring-page { max-width: 800px; margin: 0 auto; }
  ...
  .color-input { width: 3rem; height: 2.25rem; padding: 0.125rem; border: 1px solid var(--grove-overlay-12); border-radius: var(--border-radius-standard); cursor: pointer; }
  .form-actions { margin-top: 1.25rem; }
  :global(.btn-icon) { width: 1rem; height: 1rem; margin-right: 0.375rem; }
  .log-form { margin-bottom: 1rem; }
  .log-inputs { display: flex; gap: 0.5rem; align-items: center; flex-wrap: wrap; }
  .note-input { flex: 1; min-width: 150px; }
  .mood-timeline { display: flex; flex-wrap: wrap; gap: 0.375rem; padding: 0.5rem 0; }
