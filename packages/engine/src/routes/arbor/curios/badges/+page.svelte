...
<div class="badges-page">
  <header class="page-header">
    ...
    <p class="subtitle">
      Collectible achievements celebrating your milestones.
          ...
          <GlassCard class="badge-card">
            ...
            <div class="badge-details">
              ...
              <div class="badge-meta">
                <span class="rarity-tag" style="color: {getRarityColor(badge.rarity)}">{badge.rarity}</span>
            ...
            <form method="POST" action="?/toggleShowcase" use:enhance>
              ...
              <GlassButton type="submit" variant="ghost" class="showcase-btn" title={badge.isShowcased ? "Remove from showcase" : "Add to showcase"} aria-label={badge.isShowcased ? "Remove from showcase" : "Add to showcase"}>
                <Star class="btn-icon" />
              </GlassButton>
            </form>
          </GlassCard>
        ...
        <form method="POST" action="?/createCustom" use:enhance>
          <div class="form-grid">
            <div class="form-field">
              ...
              <input type="text" id="name" name="name" required maxlength="50" class="glass-input" placeholder="My Badge" />
            </div>
            <div class="form-field">
              <label for="iconUrl">Icon URL</label>
              <input type="url" id="iconUrl" name="iconUrl" required class="glass-input" placeholder="https://example.com/badge.svg" />
            </div>
            <div class="form-field full-width">
              <label for="description">Description</label>
              <input type="text" id="description" name="description" required maxlength="200" class="glass-input" placeholder="A badge for..." />
            </div>
          </div>
          <div class="form-actions">
            ...
            <GlassButton variant="ghost" onclick={() => (showCustomForm = false)}>Cancel</GlassButton>
          </div>
        </form>
      </GlassCard>
    {/if}
        ...
        {#each data.customBadges as badge}
          <GlassCard class="custom-badge-card">
            ...
            <form method="POST" action="?/removeCustom" use:enhance>
              <input type="hidden" name="badgeId" value={badge.id} />
              <GlassButton type="submit" variant="ghost" class="remove-btn" title="Remove badge" aria-label="Remove badge">
...
<style>
  ...
  h2 { font-size: 1.25rem; font-weight: 600; margin: 0 0 0.75rem 0; color: var(--color-text); }
  .section-desc { font-size: 0.85rem; color: var(--color-text-muted); margin-bottom: 0.75rem; }
  ...
  .badge-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 0.75rem; }
  :global(.badge-card) { padding: 1rem; display: flex; align-items: center; gap: 0.75rem; }
  .badge-icon-wrap { width: 48px; height: 48px; border-radius: 50%; border: 2px solid; display: flex; align-items: center; justify-content: center; flex-shrink: 0; background: var(--grove-overlay-4); }
  .badge-icon-wrap img { border-radius: 50%; }
  :global(.badge-fallback-icon) { width: 24px; height: 24px; color: var(--color-text-muted); }
  .badge-details { flex: 1; min-width: 0; }
  .badge-details h3 { font-size: 0.9rem; font-weight: 600; margin: 0; color: var(--color-text); }
  .badge-desc { font-size: 0.8rem; color: var(--color-text-muted); margin: 0.125rem 0; }
  .badge-meta { display: flex; gap: 0.5rem; align-items: center; margin-top: 0.25rem; }
  .rarity-tag { font-size: 0.7rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; }
  .showcase-tag { font-size: 0.7rem; display: inline-flex; align-items: center; gap: 0.25rem; color: var(--color-primary); }
  :global(.showcase-icon) { width: 0.7rem; height: 0.7rem; }
  :global(.showcase-btn) { min-width: 2.75rem; min-height: 2.75rem; }
  .system-badge-list { display: flex; flex-direction: column; gap: 0.375rem; }
  .system-badge-item { display: flex; align-items: center; gap: 0.75rem; padding: 0.5rem 0.75rem; background: var(--grove-overlay-4); border-radius: var(--border-radius-standard); font-size: 0.85rem; }
  .system-badge-name { font-weight: 600; color: var(--color-text); min-width: 120px; }
  .system-badge-criteria { flex: 1; color: var(--color-text-muted); }
  :global(.add-form-card) { padding: 1.5rem; margin-bottom: 1rem; }
  ...
  .custom-badge-list { display: flex; flex-direction: column; gap: 0.5rem; margin-top: 1rem; }
