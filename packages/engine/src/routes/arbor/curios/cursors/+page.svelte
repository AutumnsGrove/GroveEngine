...
<div class="cursors-page">
  <header class="page-header">
    ...
    <p class="subtitle">
      ...
      Choose a preset cursor and optionally add a trail effect.
    </p>
  </header>
...
<style>
  .cursors-page { max-width: 800px; margin: 0 auto; }
  ...
  :global(.config-card) { padding: 1.5rem; }
  h2 { font-size: 1.15rem; font-weight: 600; margin: 0 0 1rem 0; color: var(--color-text); }
  ...
  .preset-pills { display: flex; flex-wrap: wrap; gap: 0.5rem; }
  .preset-pill { padding: 0.5rem 1rem; border: 1px solid var(--grove-overlay-12); border-radius: var(--border-radius-standard); cursor: pointer; font-size: 0.85rem; color: var(--color-text); background: var(--grove-overlay-4); transition: all 0.2s; }
  .preset-pill:hover { border-color: var(--color-primary); }
  .preset-pill.selected { border-color: var(--color-primary); background: var(--grove-overlay-8); font-weight: 600; }
  .sr-only { position: absolute; width: 1px; height: 1px; padding: 0; margin: -1px; overflow: hidden; clip: rect(0,0,0,0); border: 0; }
  .divider { height: 1px; background: var(--grove-overlay-12); margin: 1.5rem 0; }
  .trail-toggle { margin-bottom: 1rem; }
  .toggle-label { display: flex; align-items: center; gap: 0.5rem; cursor: pointer; font-size: 0.9rem; color: var(--color-text); }
  .trail-options { display: flex; flex-direction: column; gap: 1rem; margin-bottom: 1rem; }
