...
import { createMockD1, createMockR2, seedMockD1, clearMockD1 } from './__mocks__/cloudflare';
import {
	// Types
	StorageError,
	type StorageFile,
...
describe('Storage Service', () => {
	...
	describe('validateFile', () => {
		...
		it('should accept file within custom size limit', () => {
			...
			expect(() => validateFile(data, 'image/png', { maxFileSize: 200 * 1024 * 1024 })).not.toThrow();
		});

		it('should reject file exceeding custom size limit', () => {
			const data = new ArrayBuffer(100 * 1024 * 1024); // 100MB
			expect(() => validateFile(data, 'image/png', { maxFileSize: 50 * 1024 * 1024 })).toThrow(StorageError);
		});

		it('should reject disallowed content type', () => {
			const data = new ArrayBuffer(1024);
			expect(() => validateFile(data, 'application/x-executable')).toThrow(StorageError);
		});
	...
	describe('isAllowedContentType', () => {
		...
		it('should accept additional types', () => {
			expect(isAllowedContentType('application/custom', ['application/custom'])).toBe(true);
		});
	});
	...
	describe('deleteFile', () => {
		it('should delete from both R2 and D1', async () => {
			...
			db.prepare = vi.fn((sql: string) => ({
				bind: () => ({
					first: vi.fn(async () => ({ id: 'del-1', key: 'images/delete-me.png' })),
		...
		it('should throw when file does not exist', async () => {
			...
			await expect(deleteFile(bucket, db, 'nonexistent/path')).rejects.toThrow(StorageError);
		});
	});
	...
	describe('listFiles', () => {
		it('should list files in a folder', async () => {
			const mockFiles = [
				{ id: '1', folder: 'images', filename: 'a.png', key: 'images/a.png', content_type: 'image/png', size_bytes: 100, original_filename: 'a.png', uploaded_by: 'user', created_at: '2024-01-01' },
				{ id: '2', folder: 'images', filename: 'b.png', key: 'images/b.png', content_type: 'image/png', size_bytes: 200, original_filename: 'b.png', uploaded_by: 'user', created_at: '2024-01-02' }
			];
	...
	describe('listAllFiles', () => {
		it('should list all files regardless of folder', async () => {
			const mockFiles = [
				{ id: '1', folder: 'images', filename: 'a.png', key: 'images/a.png', content_type: 'image/png', size_bytes: 100, original_filename: 'a.png', uploaded_by: 'user', created_at: '2024-01-01' },
				{ id: '2', folder: 'docs', filename: 'b.pdf', key: 'docs/b.pdf', content_type: 'application/pdf', size_bytes: 200, original_filename: 'b.pdf', uploaded_by: 'user', created_at: '2024-01-02' }
			];
	...
	describe('listFolders', () => {
		it('should return unique folder names', async () => {
			...
			db.prepare = vi.fn(() => ({
				all: vi.fn(async () => ({
					results: [{ folder: 'images' }, { folder: 'docs' }, { folder: 'videos' }]
				}))
			}));
	...
	describe('Constants', () => {
		it('should export default max file size', () => {
			expect(STORAGE_DEFAULTS.MAX_FILE_SIZE).toBe(50 * 1024 * 1024);
		});
	});
});
