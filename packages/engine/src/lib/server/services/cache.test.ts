...
describe('Cache Service', () => {
	...
	describe('get', () => {
		...
		it('should apply namespace prefix', async () => {
			await kv.put('grove:myns:testkey', JSON.stringify({ data: 'namespaced' }));

			const result = await get<{ data: string }>(kv, 'testkey', { namespace: 'myns' });
	...
	describe('getOrSet', () => {
		...
		it('should force refresh when specified', async () => {
			...
			const result = await getOrSet(kv, 'testkey', { compute, forceRefresh: true });
		...
		it('should throw CacheError when compute fails', async () => {
			...
			await expect(getOrSet(kv, 'key', { compute })).rejects.toThrow(CacheError);
		});
	...
	describe('getOrSetSync', () => {
		...
		it('should throw when cache write fails', async () => {
			...
			await expect(getOrSetSync(kv, 'key', { compute })).rejects.toThrow(CacheError);
		});
	});
	...
	describe('rateLimit', () => {
		...
		it('should track request count', async () => {
			// First request
			const r1 = await rateLimit(kv, 'api:endpoint', { limit: 3, windowSeconds: 60 });
			expect(r1.remaining).toBe(2);

			// Second request
			const r2 = await rateLimit(kv, 'api:endpoint', { limit: 3, windowSeconds: 60 });
			expect(r2.remaining).toBe(1);

			// Third request
			const r3 = await rateLimit(kv, 'api:endpoint', { limit: 3, windowSeconds: 60 });
		...
		it('should block requests over the limit', async () => {
			...
			const result = await rateLimit(kv, 'limited:key', { limit: 3, windowSeconds: 60 });
		...
		it('should reset after window expires', async () => {
			...
			const result = await rateLimit(kv, 'expiring:key', { limit: 3, windowSeconds: 60 });
			expect(result.allowed).toBe(true);
			expect(result.remaining).toBe(2);
		});

		it('should include resetAt timestamp', async () => {
			const result = await rateLimit(kv, 'timestamp:key', { limit: 5, windowSeconds: 300 });
