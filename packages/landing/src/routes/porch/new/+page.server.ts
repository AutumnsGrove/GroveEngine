...
import { verifyTurnstileToken, generateId } from '@autumnsgrove/groveengine/services';
...
const VALID_CATEGORIES = ['billing', 'technical', 'account', 'hello', 'other'] as const;
...
export const actions: Actions = {
	submit: async ({ request, platform, locals, getClientAddress }) => {
		...
		if (!email) {
			return fail(400, { error: 'Email is required so I can get back to you.' });
		}
		...
		if (!message || message.length < 10 || message.length > 5000) {
			return fail(400, { error: 'Please enter a message between 10 and 5000 characters.' });
		}

		// Turnstile verification for guests
		if (!locals.user) {
			if (!turnstileToken) {
				return fail(400, { error: 'Please complete the verification challenge.' });
			}
			...
			if (!turnstileResult.success) {
				return fail(403, { error: 'Human verification failed. Please try again.' });
			}
		}
		...
		const visitNumber = await generateVisitNumber(platform.env.DB, platform.env.CACHE);
		...
		try {
			...
			return fail(500, { error: 'Failed to start your visit. Please try again.' });
		}
...
---
Reply in Arbor: https://grove.place/admin/porch/${visitId}`;
...
<a href="https://grove.place/admin/porch/${visitId}" style="color: #166534;">Reply in Arbor</a>
</p>
</div>`;
