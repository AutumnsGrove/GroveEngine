...
export const actions: Actions = {
	reply: async ({ params, request, locals, platform }) => {
		...
		if (visit.status === 'resolved') {
			return fail(400, { error: 'This conversation is resolved. Start a new visit if you need help.' });
		}
		...
		try {
			...
			await platform.env.DB.prepare(
				...
				.bind(messageId, visit.id, 'visitor', locals.user.name || 'Wanderer', content, now)
				.run();
...
---
Reply in Arbor: https://grove.place/admin/porch/${visit.id}`;
...
<a href="https://grove.place/admin/porch/${visit.id}" style="color: #166534;">Reply in Arbor</a>
</p>
</div>`;
