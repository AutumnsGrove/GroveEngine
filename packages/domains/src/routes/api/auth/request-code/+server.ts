  ...
  try {
    ...
    if (RESEND_API_KEY) {
      const emailResponse = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: "Grove Domains <noreply@grove.place>",
          to: normalizedEmail,
          subject: "Your Domain Finder Login Code",
          html: `
						<div style="font-family: Georgia, serif; max-width: 480px; margin: 0 auto; padding: 40px 20px;">
							<h1 style="color: #5b21b6; font-size: 24px; margin-bottom: 8px;">Domain Finder</h1>
							<p style="color: #666; margin-bottom: 24px;">Your login code is:</p>
							<div style="background: #f5f3ff; border: 1px solid #ddd6fe; border-radius: 8px; padding: 24px; text-align: center; margin-bottom: 24px;">
								<code style="font-size: 32px; letter-spacing: 4px; color: #7c3aed; font-weight: bold;">${magicCode.code}</code>
							</div>
      ...
      if (!emailResponse.ok) {
        ...
        throw error(500, `Failed to send email: ${errorText}`);
      }
    } else {
