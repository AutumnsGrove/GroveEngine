...
function parseConfigRow(row: TenantConfigRow): TenantConfig {
  return {
    ...
    reposInclude: row.repos_include ? JSON.parse(row.repos_include) : null,
    reposExclude: row.repos_exclude ? JSON.parse(row.repos_exclude) : null,
