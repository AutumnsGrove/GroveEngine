...
export const POST: RequestHandler = async ({ request, cookies, platform, url }) => {
	...
	if (!validateOrigin(request)) {
		console.warn("[Passkey Verify] Invalid origin:", request.headers.get("origin"));
	...
	const returnTo = body.returnTo || cookies.get("passkey_return_to") || "/profile";
	...
	try {
		...
		const response = await fetch(`${authBaseUrl}/api/auth/passkey/verify-authentication`, {
			method: "POST",
			headers: {
		...
		if (!response.ok) {
			...
			const errorMessage = errorData.message || errorData.error || "Passkey verification failed";

			if (response.status === 404) {
				throw error(401, "No passkey found for this account. Try signing in with Google instead.");
			}
		...
		console.log("[Passkey Verify] Authentication successful for:", onboardingId);
