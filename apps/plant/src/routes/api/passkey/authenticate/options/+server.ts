...
export const POST: RequestHandler = async ({ request, cookies, platform }) => {
	...
	if (!validateOrigin(request)) {
		console.warn("[Passkey Auth] Invalid origin:", request.headers.get("origin"));
	...
	try {
		...
		const response = await fetch(`${authBaseUrl}/api/auth/passkey/generate-authenticate-options`, {
			method: "GET",
			headers: {
		...
		if (!response.ok) {
			...
			throw error(
				response.status,
				errorData.message || errorData.error || "Failed to get passkey authentication options",
			);
		}
