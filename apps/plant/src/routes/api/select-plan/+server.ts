...
export const POST: RequestHandler = async ({ request, cookies, platform, getClientAddress }) => {
	...
	if (shouldSkipCheckout(plan)) {
		...
		if (clientIP) {
			const ipAllowed = await checkFreeAccountIPLimit(db, clientIP).catch(() => true);
			if (!ipAllowed) {
				return json(
					{
						error:
		...
		try {
			...
			if (!existing) {
             ...
             FROM user_onboarding WHERE id = ?`,
				...
				if (!onboarding) {
					return json({ error: "Onboarding session not found." }, { status: 404 });
				}
				...
				console.log(`[Select Plan API] Created wanderer tier tenant for ${onboarding.username}`);
			}
		} catch (err) {
			logPlantError(PLANT_ERRORS.ONBOARDING_UPDATE_FAILED, {
				path: "/api/select-plan",
