...
export const POST: RequestHandler = async ({ cookies, platform }) => {
	...
	try {
		...
		return json({ error: "Unable to load your session. Please try again." }, { status: 500 });
	}

	if (!onboarding) {
		return json({ error: "Onboarding session not found. Please sign in again." }, { status: 404 });
	}
	...
	try {
		...
		return json({ error: "Unable to verify your invite. Please try again." }, { status: 500 });
	}

	if (!compedInvite) {
		return json({ error: "No valid invite found for this account." }, { status: 403 });
	}
	...
	try {
		...
		return json({ error: "Failed to create your blog. Please try again." }, { status: 500 });
	}
};
