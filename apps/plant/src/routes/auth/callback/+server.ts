...
export const GET: RequestHandler = async ({ url, cookies, platform }) => {
	...
	try {
		const sessionResponse = await platform.env.AUTH.fetch(`${authBaseUrl}/api/auth/get-session`, {
			method: "GET",
	...
	if (existingOnboarding) {
		...
		// Also check the users/tenants tables in case the onboarding record has
		// a stale NULL tenant_id (e.g., grove was created outside Plant's flow).
		...
		if (existingOnboarding.tenant_id) {
			...
			// Onboarding record exists but tenant_id is NULL — the user may have
			// created their grove outside Plant (pre-Plant users, direct setup, etc.).
			// Cross-reference the users/tenants tables to catch this case.
			try {
				const existingUser = (await db
					.prepare("SELECT tenant_id FROM users WHERE LOWER(email) = ? AND tenant_id IS NOT NULL")
					.bind(user.email.toLowerCase())
				...
				if (existingUser) {
					...
					if (tenant && /^[a-z0-9-]+$/.test(tenant.subdomain)) {
						// Backfill the onboarding record so this lookup isn't needed again
						await db
							.prepare(
				...
				// Non-fatal: if cross-reference fails, continue to onboarding steps
