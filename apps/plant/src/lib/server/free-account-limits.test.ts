...
import { checkFreeAccountIPLimit, logFreeAccountCreation } from "./free-account-limits";

/**
 * Mock D1 Database Helper
 * Creates a mock D1 with chainable prepare/bind/first/run pattern
 */
function createMockDb(firstResult: unknown = null): GroveDatabase {
	return {
...
describe("Free Account IP Limits", () => {
	...
	describe("checkFreeAccountIPLimit", () => {
		...
		it("propagates DB errors to the caller", async () => {
			...
			await expect(checkFreeAccountIPLimit(db, "10.0.0.1")).rejects.toThrow("D1 connection failed");
		});
	});
