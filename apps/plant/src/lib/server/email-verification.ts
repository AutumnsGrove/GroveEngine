...
export function normalizeEmail(email: string): string {
	...
	if (!isValidEmailFormat(normalized)) {
		throw new Error(
			`${PLANT_ERRORS.INVALID_EMAIL_FORMAT.code}: ${PLANT_ERRORS.INVALID_EMAIL_FORMAT.adminMessage}`,
		);
	}
	return normalized;
}
...
export async function verifyCode(
	...
	if (verification.code !== submittedCode) {
		...
		const remainingAttempts = MAX_ATTEMPTS - (verification.attempts as number) - 1;
		return {
			success: false,
	...
	emitPulseEvent("signup.email_verified", {
		app: "plant",
		route: "/verify-email",
