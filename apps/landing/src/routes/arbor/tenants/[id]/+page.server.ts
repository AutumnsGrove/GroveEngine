...
export const load: PageServerLoad = async ({ parent, platform, params }) => {
	...
	const tenant = await DB.prepare("SELECT * FROM tenants WHERE id = ?").bind(id).first<Tenant>();
	...
	const flagsEnv = platform.env.CACHE_KV ? { DB, FLAGS_KV: platform.env.CACHE_KV } : null;

	// Load content stats + admin data in parallel
	const [postResult, pageResult, mediaResult, storageResult, greenhouse] = await Promise.all([
		...
		DB.prepare("SELECT COUNT(*) as count FROM media WHERE tenant_id = ?")
			...
			.first<{ count: number }>()
			.catch(() => ({ count: 0 })),
		DB.prepare("SELECT COALESCE(SUM(size), 0) as total FROM media WHERE tenant_id = ?")
			.bind(id)
			.first<{ total: number }>()
			.catch(() => ({ total: 0 })),
...
export const actions: Actions = {
	enroll: async ({ request, locals, platform, params }) => {
		...
		const success = await enrollInGreenhouse(params.id, locals.user.email, notes, {
			DB: env.DB,
	...
	toggleGreenhouse: async ({ request, locals, platform, params }) => {
		...
		return {
			success: true,
			message: enabled ? "Greenhouse access enabled" : "Greenhouse access paused",
		};
	},
