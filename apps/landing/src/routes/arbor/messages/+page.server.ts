...
export const actions: Actions = {
	create: async ({ request, locals, platform }) => {
		...
		if (!channel || !(GROVE_MESSAGE_CHANNELS as readonly string[]).includes(channel)) {
	...
	update: async ({ request, locals, platform }) => {
		...
		try {
         ...
         WHERE id = ?`,
				...
				.bind(title, body, channel, message_type, pinned, expires_at || null, now, id)
				.run();
	...
	publish: async ({ request, locals, platform }) => {
		...
		try {
			const now = new Date().toISOString();
			await DB.prepare(`UPDATE grove_messages SET published = ?, updated_at = ? WHERE id = ?`)
	...
	archive: async ({ request, locals, platform }) => {
		...
		try {
			await DB.prepare("DELETE FROM grove_messages WHERE id = ?").bind(id).run();
