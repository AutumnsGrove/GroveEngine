...
export const POST: RequestHandler = async ({ request, platform }) => {
	...
	if (!authHeader?.startsWith("Bearer ") || !timingSafeEqual(authHeader.slice(7), apiKey)) {
	...
	try {
		...
		if (report.createIncident && report.results.errorRate > 0.05) {
			...
			const incidentType = report.results.errorRate > 0.5 ? "outage" : "degraded";
...
export const OPTIONS: RequestHandler = async ({ request }) => {
	...
	const isAllowed =
		origin !== null && (origin.endsWith(".grove.place") || origin === "https://grove.place");
	const allowedOrigin = isAllowed ? origin : "https://status.grove.place";
