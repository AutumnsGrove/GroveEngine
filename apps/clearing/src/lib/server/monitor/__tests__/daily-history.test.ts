...
describe("getWorstStatus", () => {
	...
	it("should return the worst status from list", () => {
		expect(getWorstStatus(["operational", "degraded", "partial_outage"])).toBe("partial_outage");
		expect(getWorstStatus(["degraded", "major_outage", "partial_outage"])).toBe("major_outage");
