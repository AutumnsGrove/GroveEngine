...
export type SearchStatus = 'pending' | 'running' | 'complete' | 'needs_followup' | 'failed';
...
export async function getSearchJob(db: D1Database, id: string): Promise<DomainSearchJob | null> {
	const result = await db
...
export async function listSearchJobs(
	...
	const [jobsResult, countResult] = await Promise.all([
		db.prepare(query).bind(...params).all<DomainSearchJob>(),
...
export async function updateSearchJobStatus(
	...
	if (updates.completed_at !== undefined) {
		...
		fields.push('duration_seconds = CAST((julianday(?) - julianday(started_at)) * 86400 AS INTEGER)');
		values.push(updates.completed_at);
	}
...
export async function getJobResults(
	...
	const result = await db.prepare(query).bind(...params).all<DomainResult>();
...
export async function getActiveConfig(db: D1Database): Promise<SearchConfig | null> {
	const result = await db
		.prepare('SELECT * FROM domain_search_config WHERE is_active = 1 ORDER BY updated_at DESC LIMIT 1')
		.first<SearchConfig>();
	return result ?? null;
}
...
export async function updateConfig(
	...
	await db
		.prepare(`UPDATE domain_search_config SET ${fields.join(', ')} WHERE id = ?`)
		.bind(...values)
		.run();
}
